...
select prov from suministro inner join proyecto on suministro.proyecto=proyecto.proyecto where proyecto.nombre='Programacion';
...
select proveedor.prov,proyecto.proyecto,pieza.pieza from proveedor join proyecto join pieza join suministro on proyecto.proyecto=suministro.proyecto and proveedor.prov=suministro.prov and pieza.pieza=suministro.pieza and pieza.ciudad=proyecto.ciudad and proyecto.ciudad=proveedor.ciudad;

#4.	Encontrar los códigos y nombres de los proyectos que tienen suministro cuya cantidad es superior a 1000.
select suministro.proyecto, proyecto.nombre, suministro.cantidad from proyecto join suministro on suministro.proyecto=proyecto.proyecto where cantidad >=1000;

#5.	Encontrar los proveedores que suministran a proyectos de Programación pero no de Construcción.
select  proveedor.prov from proveedor join suministro join proyecto on suministro.proyecto=proyecto.proyecto and suministro.prov=proveedor.prov where proyecto.nombre='Programacion' and proveedor.prov not in(select proveedor.prov from proveedor join suministro join proyecto on suministro.proyecto=proyecto.proyecto and suministro.prov=proveedor.prov where proyecto.nombre='Construccion')  ;
...
select proyecto.nombre,pieza.nombre from proyecto join pieza join suministro on suministro.proyecto=proyecto.proyecto and pieza.pieza=suministro.pieza and suministro.cantidad>100;
