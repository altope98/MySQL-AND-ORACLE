...
# Los estudiantes no han realizado ninguna solicitud
select sName, student.sID, cName, major
from Student left outer join Apply using(sID);

#nombre de las universidades que no han sido solicitadas
select college.cname from apply right join college on apply.cname=college.cname where sID is null;
insert into college values('Granada','CS',15000);
select college.cname from college left join apply on apply.cname=college.cname where sID is null;
show tables from video;
select * from student;
select * from college;
select * from apply;
# estudiantes que no han solicitado universidad y universidades que no han recibido solicitud
select apply.sid,college.cname from apply right join college on apply.cname=college.cname where apply.sID is null
union
select student.sName, apply.cname from student left join apply on apply.sID=student.sID where Cname is null;

# Estudiantes y sus solicitudes y universidades con sus solicitudes
select apply.sid,college.cname from apply right join college on apply.cname=college.cname
union
select student.sName, apply.cname from student left join apply on apply.sID=student.sID ;
